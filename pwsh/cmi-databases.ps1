...
function CreateDbList {
    ...
    try {
        $jsonData += Get-CMI-Config-Data -u "http://localhost:5001/api/data/cmi/${Env}"
        $jsonData += Get-CMI-Config-Data -u "http://localhost:5001/api/data/ais/${Env}"
...
if ($Job -eq "list") {
    ...
    Write-Output (CreateDbList -Env $Env)
